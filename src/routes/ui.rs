...
const INDEX: &str = r##"<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Hotarun</title><link rel="stylesheet" href="/ui/style.css"></head><body><header><h1>Hotarun</h1><nav><a href="/ui/">Dashboard</a><a href="/ui/tuners">Tuners</a><a href="/ui/channels">Channels</a><a href="/ui/logs">Logs</a><a href="/ui/configuration">Configuration</a></nav></header><main id="app"><p>Loading…</p></main><script src="/ui/app.js"></script></body></html>"##;
            ...
            for (const name of ['channels', 'tuners', 'server']) {
                ...
                const save = button('Save ' + name, async () => {
                    ...
                    try {
                        ...
                        if (!response.ok) {
                            const detail = await response.text();
                            status.textContent = 'Save failed: ' + (detail || response.statusText);
                            return;
                        }
                        const result = await response.json();
                        status.textContent = result.restartRequired
                            ? 'Saved ' + name + '. Restart required.'
                            : 'Saved ' + name + '.';
                    } catch (error) {
                        status.textContent = 'Save status unknown: ' + String(error);
                    } finally {
                        save.disabled = false;
                    }
                });
                app.append(text('h3', name), area, save, status);
            }
            const restart = restartControl();
            app.append(text('h3', 'Apply saved configuration'), restart[0], restart[1]);
        } else {
...
pub fn router() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route("/ui/tuners", get(index))
        .route("/ui/channels", get(index))
        .route("/ui/logs", get(index))
        .route("/ui/configuration", get(index))
}
...
mod tests {
    ...
    #[test]
    fn ui_renders_values_as_text_and_exposes_save_actions_without_scan_controls() {
        assert!(!SCRIPT.contains("innerHTML"));
        assert!(SCRIPT.contains("textContent"));
        assert!(SCRIPT.contains("method: 'PUT'"));
        assert!(SCRIPT.contains("b.type = 'button'"));
        assert!(!INDEX.contains("/ui/scan"));
        assert!(!SCRIPT.contains("channels/scan"));
    ...
    #[test]
    fn configuration_save_reports_results_and_has_one_restart_control() {
        assert!(SCRIPT.contains("const status = text('p', '')"));
        assert!(SCRIPT.contains("const result = await response.json()"));
        assert!(SCRIPT.contains("result.restartRequired"));
        assert!(SCRIPT.contains("if (!response.ok)"));
        assert!(SCRIPT.contains("const detail = await response.text()"));
        assert!(SCRIPT.contains("status.textContent = 'Save status unknown: ' + String(error)"));
        assert!(SCRIPT.contains("save.disabled = true"));
        assert!(SCRIPT.contains("finally {\n                        save.disabled = false;"));
        assert!(SCRIPT.contains("const restart = restartControl();\n            app.append(text('h3', 'Apply saved configuration'), restart[0], restart[1]);"));
    }
}
